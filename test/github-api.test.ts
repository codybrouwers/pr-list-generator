...
// Mock Octokit class with flexible types
class MockOctokit {
  rest = {
    users: {
...
describe("GitHub API Integration", () => {
  ...
  describe("Pull Request Fetching", () => {
    test("should fetch pull requests for a repository", async () => {
      ...
      mockOctokit.rest.pulls.list = mock(() => Promise.resolve({ data: mockPRs as any[] }));

      const { data: prs } = await mockOctokit.rest.pulls.list();

      expect(prs).toHaveLength(2);
      expect((prs as any[])[0].number).toBe(123);
      expect((prs as any[])[1].number).toBe(456);
    ...
    test("should handle empty PR list", async () => {
      mockOctokit.rest.pulls.list = mock(() => Promise.resolve({ data: [] as any[] }));
  ...
  describe("Pull Request Details", () => {
    test("should fetch detailed PR information", async () => {
      ...
      mockOctokit.rest.pulls.get = mock(() => Promise.resolve({ data: mockDetailedPR as any }));

      const { data: pr } = await mockOctokit.rest.pulls.get();

      expect((pr as any).number).toBe(123);
      expect((pr as any).additions).toBe(50);
      expect((pr as any).deletions).toBe(10);
      expect(mockOctokit.rest.pulls.get).toHaveBeenCalledTimes(1);
    });

    test("should handle missing additions/deletions", async () => {
      const mockPR = {
        number: 123,
        title: "Test PR",
        html_url: "https://github.com/test/repo/pull/123"
        // additions and deletions are undefined
      };

      mockOctokit.rest.pulls.get = mock(() => Promise.resolve({ data: mockPR as any }));

      const { data: pr } = await mockOctokit.rest.pulls.get();

      // Should handle undefined values gracefully
      const additions = (pr as any).additions || 0;
      const deletions = (pr as any).deletions || 0;
    ...
    test("should handle API errors when fetching PR details", async () => {
      mockOctokit.rest.pulls.get = mock(() =>
  ...
  describe("Repository Parsing", () => {
    test("should parse repository owner and name", () => {
      ...
      testCases.forEach(({ repo, owner, name }) => {
